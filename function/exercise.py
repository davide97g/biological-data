...
df.to_csv("../data/function/"+user+"/uniprot_ids.csv", index=False)
...
if __name__ == "__main__":
    ...
    answers += "\te) " + \
        str(len([leaf for leaf in leaf_terms if leaf in terms_set]))+"\n"
    ...
    for root in roots:
        ...
        for (k, v) in list(filter(lambda x: graph[x[0]]["namespace"] == graph[root]["namespace"], data))[:10]:
            ...
            answers += "\t\t " + str(k) + " "+str(v) + " " + graph[k]["def"]
    ...
    print("\n\tb)")
    answers += "\n## Q5\n"
    answers += "\n\ta) " + str(len([x for x in data if x[5] > 1]))
    answers += "\n\tb)"
    for root in roots:
        print("\n\t", graph[root]["namespace"])
        answers += "\n\t " + graph[root]["namespace"]
    ...
    print("\tb)\n")
    answers += "\n## Q6\n"
    answers += "\ta) The enriched terms are the ones with low right tail values (usually is <0.05)"
    answers += "\tb)\n"
    for root in roots:
        print("\n\t", graph[root]["namespace"])
        answers += "\n\t " + graph[root]["namespace"]
    ...
    with open("../.docs/midterm 3/"+user+"/answers.md", "w") as f:
        f.write(answers)

    # for term in terms_set:
    #     ratio_set = (terms_set[term] + 1) / \
    #         proteins_set  # add pseudo count
    #     ratio_rest = terms_rest.get(
    #         term, 1) / proteins_rest  # add pseudo count
    #     fold_increase = ratio_set / ratio_rest
    #     if fold_increase > 1:
    #         p = pvalue(terms_set[term] + 1, proteins_set,
    #                    terms_rest.get(term, 1), proteins_rest)
    #         if p.right_tail < 0.05:
